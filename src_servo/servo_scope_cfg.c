			...
			if (argument != NULL)
			{
				...
				switch (tokenEnumValue)
				{
					case MC_FREQ:
						...
						*freq	=	atof(argument);
						// parameter is ok, print it out
						printf("%-15.15s = %-15.4f  ", token, *freq);
					...
					#else
						strcpy(port, argument);
						printf("%-15.15s = %-15.15s  ", token, port);
					#endif
						break;
...
#else
	Servo_Read_Scope_Cfg(	configFile,
							&ra,
							&dec,
							&freq,
							&addr,
