...
//*	Jul  6,	2022	<MLS> Added Servo_check_observ_cfg()
//****************************************************************************
...
TYPE_OBSERV_CFG gServoObservCfg;


//******************************************************************
TYPE_CFG_ITEM gObservConfigArray[] =
{
	{"EPOCH:",			EPOCH,			false},
...
{
	return(gServoObservCfg.lat);
}
//*****************************************************************************
void Servo_set_lat(double newLatValue)
{
	gServoObservCfg.lat	=	newLatValue;
}
...
{
	return(gServoObservCfg.lon);
}
//*****************************************************************************
void Servo_set_lon(double newLonValue)
{
	gServoObservCfg.lon	=	newLonValue;
}
...
{
	return(gServoObservCfg.elev);
}
//*****************************************************************************
void Servo_set_elev(double newElevValue)
{
	gServoObservCfg.elev	=	newElevValue;
}
...
{
	return(gServoObservCfg.temp);
}
//*****************************************************************************
double Servo_get_pressure(void)
{
	return(gServoObservCfg.press);
}
...
int		enumValue;
	...
	iii			=	0;
	enumValue	=	-1;
	while ((enumValue < 0) && (gObservConfigArray[iii].enumValue >= 0))
	{
		if (strcasecmp(keyword, gObservConfigArray[iii].parameter) == 0)
		{
			enumValue	=	gObservConfigArray[iii].enumValue;
							   ...
							   char *cfgErrorString3)
{
	...
	{
	case EPOCH:
		gObservConfigArray[EPOCH].found	=	true;
	...
	case EPOCH_JD:
		gObservConfigArray[EPOCH_JD].found	=	true;
	...
	case LATITUDE:
		gObservConfigArray[LATITUDE].found	=	true;
	...
	case LONGITUDE:
		gObservConfigArray[LONGITUDE].found	=	true;
	...
	case ELEVATION:
		gObservConfigArray[ELEVATION].found	=	true;
	...
	case TEMPERATURE:
		gObservConfigArray[TEMPERATURE].found	=	true;
	...
	case PRESSURE:
		gObservConfigArray[PRESSURE].found	=	true;
	...
	case SITE:
		gObservConfigArray[SITE].found	=	true;
...
TYPE_OBSERV_CFG	*observConfig;
	...
	observConfig	=	&gServoObservCfg;

	retStatus		=	-1;
	//*	first, go through and invalidate all of the parameters
	iii	=	0;
	while (strlen(gObservConfigArray[iii].parameter) > 0)
	{
		gObservConfigArray[iii++].found	=	false;
	}
...
static void PrintConfigParam_Dbl(const int cfgEnum, const double value)
{
	if ((cfgEnum >= 0))
	{
		printf("%-15.15s = %-15.4f  \n", gObservConfigArray[cfgEnum].parameter, value);
	}
}
...
void Print_observ_configuration(void)
{
	...
	PrintConfigParam_Dbl(EPOCH,			gServoObservCfg.baseEpoch);
	PrintConfigParam_Dbl(EPOCH_JD,		gServoObservCfg.baseJd);
	PrintConfigParam_Dbl(LATITUDE,		gServoObservCfg.lat);
	PrintConfigParam_Dbl(LONGITUDE,		gServoObservCfg.lon);
	PrintConfigParam_Dbl(ELEVATION,		gServoObservCfg.elev);
	PrintConfigParam_Dbl(TEMPERATURE,	gServoObservCfg.temp);
	PrintConfigParam_Dbl(PRESSURE,		gServoObservCfg.press);
	PrintConfigParam_Str(SITE,			gServoObservCfg.site);
}
...
char configFile[]	=	"servo_observatory.cfg";
	...
	Servo_read_observ_cfg(configFile, &gServoObservCfg);
